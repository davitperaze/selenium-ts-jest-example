...
createPageTests("Timeline", TimelinePage, "timeline", "Timeline", (getPage) => {
    test(`Timeline hero content title should be: ${strings.heroContent.timeline?.title}`, async () => {
        const title = await getPage().getMainTitleText();
        expect(title).toBe(strings.heroContent.timeline?.title);
    });

    test(`Sub title should be: ${strings.heroContent.timeline?.subtitle}`, async () => {
        const subTitle = await getPage().getSubTitleText();
        expect(subTitle).toBe(strings.heroContent.timeline?.subtitle);
    });

    test(`Timeline container should be shown`, async () => {
    ...
    test(`Each timeline card should show year, title, and description`, async () => {
